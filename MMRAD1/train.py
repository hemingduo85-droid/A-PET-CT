...
from model import MMRAD_ARCHITECTURE_VERSION, MultiModalAnomalyDetector
...
def load_model_from_checkpoint(args, device):
    ...
    version = ckpt.get("architecture_version") if isinstance(ckpt, dict) else None
    if version != MMRAD_ARCHITECTURE_VERSION or "state_dict" not in ckpt:
        raise RuntimeError(
            f"Incompatible MMRAD checkpoint: {ckpt_path}. "
            f"Expected architecture version {MMRAD_ARCHITECTURE_VERSION}, got {version!r}. "
            "Retrain the model with the corrected implementation."
        )
...
def train_model(args):
    ...
    torch.save(
        {
            "architecture_version": MMRAD_ARCHITECTURE_VERSION,
