...
def write_metrics(metrics_file, metrics):
    def convert(value):
...
def compute_metrics(labels, scores, masks, maps, filenames=None, **kwargs):
    if filenames is None:
        filenames = [f"unknown__unknown__{idx}" for idx in range(len(labels))]
...
def _bootstrap_ci(y_true, y_score, metric_fn, bootstrap_iters=500, seed=42):
    ...
    if y_true.size == 0 or len(np.unique(y_true)) < 2 or bootstrap_iters <= 0:
        value = float(metric_fn(y_true, y_score)) if y_true.size else 0.0
        return (value, value)
    ...
    for _ in range(int(bootstrap_iters)):
        ...
        if len(np.unique(sample_y)) < 2:
            continue
        values.append(float(metric_fn(sample_y, y_score[idx])))
    if not values:
...
def _pixel_slice_histogram_bootstrap_ci(
    gt_labels,
    gt_masks,
    maps,
    bootstrap_iters=500,
    ...
    if len(abn_idx) == 0 or bootstrap_iters <= 0:
        return (0.0, 0.0), (0.0, 0.0)

    masks = np.asarray(gt_masks)
    scores = np.asarray(maps, dtype=np.float32)
    score_min = float(np.nanmin(scores[keep]))
    score_max = float(np.nanmax(scores[keep]))
    ...
    for row, idx in enumerate(abn_idx):
        ...
        valid = np.isfinite(slice_scores)
        slice_mask = masks[idx].reshape(-1).astype(bool)[valid]
        slice_scores = slice_scores[valid]
    ...
    for _ in range(int(bootstrap_iters)):
        ...
        weights = np.bincount(sampled, minlength=n).astype(np.uint32)
        ...
        if progress_callback is not None and (done % 50 == 0 or done == int(bootstrap_iters)):
            progress_callback(f"Pixel histogram slice bootstrap: {done}/{int(bootstrap_iters)}")

    return (
        _clip_ci(*np.percentile(aurocs, [2.5, 97.5])),
        _clip_ci(*np.percentile(auprs, [2.5, 97.5])),
    )
...
def eval_protocol_compute_metrics(
    ...
    slice_metrics["img_auroc_ci"] = _bootstrap_ci(
        gt_labels, image_scores, safe_auroc, bootstrap_iters, ci_seed + 10
    )
    slice_metrics["img_ap_ci"] = _bootstrap_ci(
        gt_labels, image_scores, safe_ap, bootstrap_iters, ci_seed + 11
    )
    slice_metrics["img_f1_ci"] = _bootstrap_ci(
        gt_labels, image_scores, f1_score_max, bootstrap_iters, ci_seed + 12
    )
    if progress_callback is not None:
        progress_callback(eval_protocol_format_image_metrics(slice_metrics))

    abn_masks = gt_masks[gt_labels == 1]
    abn_maps = maps[gt_labels == 1]
    gt_px_abn = abn_masks.reshape(-1).astype(np.uint8)
    pr_px_abn = abn_maps.reshape(-1).astype(np.float64)
    if progress_callback is not None:
    ...
    px_auroc_abn_ci, px_aupr_abn_ci = _pixel_slice_histogram_bootstrap_ci(
        gt_labels,
        gt_masks,
        maps,
        bootstrap_iters,
    ...
    if progress_callback is not None:
        progress_callback("[Slice-Px(abn)]  " + "  ".join([
            _format_metric("AUROC", px_auroc_abn, px_auroc_abn_ci, 100.0),
            _format_metric("AUPR", px_aupr_abn, px_aupr_abn_ci, 100.0),
        ]))
    ...
    pat_metrics = {
        ...
        "pat_auroc_ci": _bootstrap_ci(pat_label, pat_score, safe_auroc, bootstrap_iters, ci_seed + 30),
