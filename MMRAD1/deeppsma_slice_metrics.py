...
def top_percent_mean(maps, percent=1.0):
    maps = np.asarray(maps, dtype=np.float32)
    flat = maps.reshape(maps.shape[0], -1)
    ratio = float(percent) / 100.0 if float(percent) > 0 else 0.01
    k = max(1, int(flat.shape[1] * ratio))
    return np.partition(flat, flat.shape[1] - k, axis=1)[:, -k:].mean(axis=1)


def compute_slice_only_metrics(labels, masks, maps, scores=None, bootstrap_iters=500, hist_bins=16384, seed=20260717):
    ...
    scores = top_percent_mean(maps, 1.0) if scores is None else np.asarray(scores, dtype=np.float64)
...
def compute_slice_only_metrics_from_cache(cache_path, bootstrap_iters=500, hist_bins=16384, seed=20260717):
    ...
    return compute_slice_only_metrics(
        ...
        cache["masks"],
        cache["maps"],
        cache["image_scores"].astype(np.float64) if "image_scores" in cache else None,
