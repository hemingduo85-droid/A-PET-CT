...
class DeResNet(nn.Module):#无跳跃连接
    def __init__(self, block, layers, num_classes=1, norm_layer=None):
        ...
        # 解码器层定义（不再需要skip连接调整）
        self.layer1 = self._make_layer(block, 256, layers[3], stride=2)
        self.layer2 = self._make_layer(block, 128, layers[2], stride=2)
        self.layer3 = self._make_layer(block, 64, layers[1], stride=2)
    ...
    def forward(self, x, skip_features=None):
        ...
        # 第一层解码
        d1 = self.layer1(x)  # [B, 2048, 16, 16]
        # print("第一次解码:", d1.shape)

        # 第二层解码
        d2 = self.layer2(d1)  # [B, 512, 32, 32]
        # print("第二次解码:", d2.shape)

        # 第三层解码
        d3 = self.layer3(d2)  # [B, 256, 64, 64]
        # print("第三次解码:", d3.shape)

        # 最终输出
        output = self.layer4(d3)
...
class MultimodalResNet(nn.Module):
    def __init__(self, modalities: List[str], pretrained: bool = True):
        ...
        if pretrained:
            # Convert the pretrained RGB stem to one channel deterministically.
            state_dict = load_state_dict_from_url(model_urls['resnet50'], progress=True)
            state_dict = prepare_grayscale_resnet_state(state_dict)
...
class ESC_DRKD_Multimodal(FrozenTeacherTrainMixin, nn.Module):
