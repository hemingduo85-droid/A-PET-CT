...
"""
Run example:
cd /data/cyf/codes/A-PET-CT/ESC_DRKD_successful
...
from train import fuse_modalities, get_anomaly_map, parse_modalities
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "ESC-DRKD")
...
def checkpoint_for_tracer(args, tracer: str, modalities) -> Path:
    explicit = args.psma_checkpoint if tracer == "PSMA" else args.fdg_checkpoint
    if explicit:
        return resolve_path(explicit)
    checkpoint_dir = resolve_path(args.checkpoint_dir) / tracer.upper() / "_".join(modalities)
    ...
    if candidates:
        return candidates[-1]
    raise FileNotFoundError(f"No checkpoint found in {checkpoint_dir}; provide --{tracer.lower()}_checkpoint")


def load_model_for_tracer(args, tracer: str, modalities, device):
    ckpt_path = checkpoint_for_tracer(args, tracer, modalities)
...
def build_input(paths, modalities, image_size, device):
    arrays = {mod: read_gray(paths[mod], image_size) for mod in modalities}
    x_dict = {
        mod: torch.from_numpy(arrays[mod]).float().unsqueeze(0).unsqueeze(0).to(device)
        for mod in modalities
    }
    return x_dict, arrays


def render_case(args, model, ckpt_path, modalities, device, data_root, group, tracer, patient, slice_id):
    paths = case_paths(data_root, tracer, patient, slice_id)
    check_paths(paths)
    x_dict, arrays = build_input(paths, modalities, args.image_size, device)
    with torch.no_grad():
        ...
        raw_map = get_anomaly_map(model, t_features, reconstruction, args.image_size).squeeze().detach().cpu().numpy()
    ...
    save_image(case_dir / "ESC-DRKD_heatmap.png", heatmap_img, args.image_size)
    ...
    return {"meta": meta, "panels": {"PET": pet_img, "CT": ct_img, "GT": gt_img, "Pred": pred_img, "ESC-DRKD": heatmap_img}}
...
def main():
    ...
    if set(modalities) != {"ct", "pet"}:
        raise ValueError("This fixed-case script expects --modalities ct,pet.")
    device = torch.device(args.device if torch.cuda.is_available() or not args.device.startswith("cuda") else "cpu")
    models = {tracer: load_model_for_tracer(args, tracer, modalities, device) for tracer in TRACERS}
    records_by_group = {group: [] for group in GROUPS}
    for group in GROUPS:
        for tracer in TRACERS:
            patient, slice_id = FIXED_CASES[group][tracer]
            model, ckpt_path = models[tracer]
            records_by_group[group].append(render_case(args, model, ckpt_path, modalities, device, data_root, group, tracer, patient, slice_id))
