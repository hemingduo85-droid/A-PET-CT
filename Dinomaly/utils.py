...
def f1_score_max(y_true, y_score):
    ...
    f1s = 2 * precs * recs / (precs + recs + 1e-7)
    f1s = f1s[:-1]
    return float(f1s.max()) if len(f1s) else 0.0
...
def _to_uint8_image(channel):
    channel = np.asarray(channel, dtype=np.float32)
    cmin, cmax = channel.min(), channel.max()
    channel = (channel - cmin) / (cmax - cmin + 1e-8)
    return np.uint8(channel * 255)


def _safe_filename(path):
    base = os.path.splitext(os.path.basename(str(path)))[0]
    patient = os.path.basename(os.path.dirname(os.path.dirname(str(path))))
    return f"{patient}_{base}"


def _save_petct_anomaly_images(records, vis_dir, vis_num):
    if not vis_dir or vis_num <= 0:
        return 0
    os.makedirs(vis_dir, exist_ok=True)
    top_records = sorted(records, key=lambda item: item["score"], reverse=True)[:vis_num]
    for rank, rec in enumerate(top_records, 1):
        stem = f"top{rank:02d}_label{rec['label']}_score{rec['score']:.4f}_{_safe_filename(rec['path'])}"
        image = _to_uint8_image(rec["image"])
        image_rgb = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
        heatmap = cvt2heatmap(_to_uint8_image(rec["anomaly"]))
        overlay = cv2.addWeighted(image_rgb, 0.55, heatmap, 0.45, 0)
        gt = _to_uint8_image(rec["gt"])
        cv2.imwrite(os.path.join(vis_dir, f"{stem}_heatmap.png"), heatmap)
        cv2.imwrite(os.path.join(vis_dir, f"{stem}_overlay.png"), overlay)
        cv2.imwrite(os.path.join(vis_dir, f"{stem}_gt.png"), gt)
    return len(top_records)


def evaluation_petct(
    model,
    dataloader,
    device,
    max_ratio=0.01,
    ...
    """Evaluate PET-CT with the same slice, abnormal-pixel, and patient metrics as PhyTwin."""
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
    ...
    total_samples = len(dataloader.dataset) if hasattr(dataloader, "dataset") else None
    seen_samples = 0
    with torch.no_grad():
        for batch_idx, (img, gt, label, path) in enumerate(dataloader, 1):
            img = img.to(device)
            en, de = model(img)

            anomaly_map, _ = cal_anomaly_maps(en, de, img.shape[-1])
            anomaly_map = gaussian_kernel(anomaly_map)   # [B,1,H,W]

            gt = gt.to(device).bool()
            if gt.shape[1] > 1:
                gt = torch.max(gt, dim=1, keepdim=True)[0]
            ...
            if max_ratio > 0:
                ...
                sp_score = flat.max(dim=1)[0]
            pr_list_sp.append(sp_score.cpu())
            seen_samples += int(img.shape[0])

            if vis_dir and vis_num > 0:
                img_for_vis = img.detach().cpu()[:, 0]
                gt_for_vis = gt.detach().cpu()[:, 0]
                anomaly_for_vis = anomaly_map.detach().cpu()[:, 0]
                for i in range(anomaly_for_vis.shape[0]):
                    records.append({
                        "score": float(sp_score.detach().cpu()[i]),
                        "label": int(label.detach().cpu()[i]),
                        "path": path[i],
                        "image": img_for_vis[i].numpy(),
                        "gt": gt_for_vis[i].numpy(),
                        "anomaly": anomaly_for_vis[i].numpy(),
                    })

            if progress_fn is not None and progress_interval and batch_idx % progress_interval == 0:
                if total_batches is not None:
                    progress_fn(
                        f"Eval/export progress: batch {batch_idx}/{total_batches}, "
                        f"samples {seen_samples}/{total_samples}"
                    )
                else:
                    progress_fn(f"Eval/export progress: batch {batch_idx}, samples {seen_samples}")

    gt_px = torch.cat(gt_list_px, dim=0)[:, 0].numpy()
    pr_px = torch.cat(pr_list_px, dim=0)[:, 0].numpy()
    gt_sp = torch.cat(gt_list_sp).numpy()
    pr_sp = torch.cat(pr_list_sp).numpy()

    if cache_path:
        if progress_fn is not None:
            progress_fn(f"Writing eval cache to {cache_path}")
        os.makedirs(os.path.dirname(cache_path) or ".", exist_ok=True)
        np.savez_compressed(
            cache_path,
            labels=gt_sp.astype(np.int32),
            masks=gt_px.astype(np.float32),
            maps=pr_px.astype(np.float32),
            image_scores=pr_sp.astype(np.float64),
            paths=np.asarray(paths, dtype=object),
        )

    saved_vis = _save_petct_anomaly_images(records, vis_dir, vis_num)
    if export_only:
        return None, None, saved_vis
    ...
    slice_metrics, patient_metrics = compute_petct_metrics(
        ...
        seed=ci_seed,
        pixel_bootstrap_max_samples=ci_pixel_max_samples,
        exact_pixel_points=exact_pixel_points,
        exact_pixel_auroc=exact_pixel_auroc,
        exact_pixel_aupr=exact_pixel_aupr,
        progress_fn=progress_fn,
    )
    return slice_metrics, patient_metrics, saved_vis
