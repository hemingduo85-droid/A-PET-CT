#!/usr/bin/env python3
"""DeepPSMA generalization commands (Dinomaly):
python export_deeppsma_figure_scores.py --source_dataset PSMA --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --input_mode dual --gpu 4 --num_epochs 30 --output_dir generalization_outputs/PSMA_weights/figure_scores --skip_existing --skip_require_pixel

python export_deeppsma_figure_scores.py --source_dataset FDG --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --input_mode dual --gpu 5 --num_epochs 30 --output_dir generalization_outputs/FDG_weights/figure_scores --skip_existing --skip_require_pixel
"""
# Run examples:
#   python export_deeppsma_figure_scores.py --input_mode dual --gpu 4 --num_epochs 30 --data_path /data/cyf/shared_data/A-PETCT/2d_equal_mask50/PSMA --save_name pet_psma --skip_existing --skip_require_pixel
#   python export_deeppsma_figure_scores.py --input_mode dual --gpu 4 --num_epochs 30 --data_path /data/cyf/shared_data/A-PETCT/2d_equal_mask50/FDG --save_name pet_fdg --skip_existing --skip_require_pixel
    ...
    with torch.no_grad():
        for _batch_idx, (img, gt, label, path) in enumerate(tqdm(test_loader, desc="Exporting Dinomaly scores")):
            ...
            for map_i, mask_i, label_i, path_i in zip(anomaly_map, masks, labels, paths):
                ...
                pix = pixel_metric_row(args.method, "DeepPSMA", args.input_mode, case_id, slice_id, path_i, map_i, mask_i)
                if pix is not None:
...
def parse_args():
    ...
    parser.add_argument("--data_path", default=None, help=argparse.SUPPRESS)
    ...
    args.original_data_root = os.path.abspath(os.path.expanduser(args.data_root))
    args.data_path = prepare_deeppsma_view(
        args.original_data_root,
        os.path.join(
            args.output_dir or GENERALIZATION_OUTPUT_TEMPLATE.format(source_dataset=args.source_dataset),
            "_deeppsma_view",
        ),
    )
    if args.save_name is None:
        args.save_name = "petct_psma" if args.source_dataset == "PSMA" else "petct_fdg"
    if args.ckpt_path is None:
        args.ckpt_path = os.path.join(args.save_dir, args.save_name, f"epoch_{args.num_epochs:02d}_model.pth")
    if args.output_dir is None:
        args.output_dir = GENERALIZATION_OUTPUT_TEMPLATE.format(source_dataset=args.source_dataset)
    return args
