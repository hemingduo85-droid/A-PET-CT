...
python export_fixed_lesion_case_figure.py \
  ...
  --image_size 256 \
  --crop_size 252 \
  --psma_save_name petct_psma \
  --fdg_save_name petct_fdg \
  --output_dir fixed_lesion_case_figures
"""
"""Render fixed PSMA/FDG lesion-size cases for Dinomaly comparison figures."""
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "Dinomaly")
...
def mask_image(mask, color=(255, 128, 0)):
...
def ckpt_for_tracer(args, tracer: str) -> Path:
    explicit = args.psma_ckpt if tracer == "PSMA" else args.fdg_ckpt
    if explicit:
        return resolve_path(explicit)
    save_name = args.psma_save_name if tracer == "PSMA" else args.fdg_save_name
    if not save_name:
        save_name = f"dinomaly_petct_{args.input_mode}_ep{args.num_epochs}_s{args.seed}"
    return resolve_path(args.save_dir) / save_name / f"epoch_{args.num_epochs:02d}_model.pth"


def load_model_for_tracer(args, tracer: str, device):
    ckpt_path = ckpt_for_tracer(args, tracer)
    if not ckpt_path.exists():
        raise FileNotFoundError(f"Checkpoint not found for {tracer}: {ckpt_path}")
    model, _trainable = build_model(device)
    checkpoint = load_checkpoint(model, str(ckpt_path), device)
    ckpt_mode = checkpoint.get("input_mode") if isinstance(checkpoint, dict) else None
    if ckpt_mode and ckpt_mode != args.input_mode:
        print(f"Warning: {tracer} checkpoint input_mode={ckpt_mode}, current input_mode={args.input_mode}")
    model.eval()
    return model, ckpt_path


def build_input(paths: dict[str, Path], input_mode: str, transform):
    ...
    if input_mode == "ct":
        ...
        raise ValueError("input_mode must be ct, pet, or dual")
...
def render_case(args, model, ckpt_path: Path, gaussian_kernel, transform, device, data_root: Path,
    ...
    image = build_input(paths, args.input_mode, transform).to(device)
    with torch.no_grad():
        en, de = model(image)
        anomaly_map, _ = cal_anomaly_maps(en, de, image.shape[-1])
    ...
    save_image(case_dir / "Dinomaly_heatmap.png", heatmap_img, args.image_size)
...
def parse_args():
    ...
    parser.add_argument("--psma_save_name", default="pet_psma")
    parser.add_argument("--fdg_save_name", default="pet_fdg")
    parser.add_argument("--num_epochs", type=int, default=30)
    parser.add_argument("--psma_ckpt", default=None)
    parser.add_argument("--fdg_ckpt", default=None)
...
def main():
    ...
    if args.crop_size % 14 != 0:
        raise ValueError("--crop_size must be divisible by 14 for Dinomaly ViT patches.")
