...
"""
Run example:
cd /data/cyf/codes/A-PET-CT/MMRAD_successful
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "MMRAD")
...
def build_input(paths, modalities, image_size):
    ...
    x = np.stack([images[mod] for mod in modalities], axis=0).astype(np.float32)
    return torch.from_numpy(x).unsqueeze(0), images


def load_model_for_tracer(args, tracer, modalities, device):
    ns = argparse.Namespace(
        checkpoint=args.psma_checkpoint if tracer == "PSMA" else args.fdg_checkpoint,
    ...
    model = load_model(ns, modalities, device)
    ckpt = checkpoint_path(ns, modalities)
...
def render_case(args, model, ckpt_path, modalities, device, data_root, group, tracer, patient, slice_id):
    ...
    with torch.no_grad():
        recon = model(image)
        raw_map = F.mse_loss(recon, image, reduction="none").mean(dim=1).squeeze(0)
        raw_map = raw_map.detach().cpu().numpy().astype(np.float32)
    ...
    save_image(case_dir / "MMRAD_heatmap.png", panels["MMRAD"], args.image_size)
    meta = {
        "method": METHOD,
        "group": group,
        "tracer": tracer,
...
def main():
    ...
    bundles = {tracer: load_model_for_tracer(args, tracer, modalities, device) for tracer in TRACERS}
    all_records = []
    for group in GROUPS:
        group_records = []
        for tracer in TRACERS:
            ...
            record = render_case(args, model, ckpt, modalities, device, data_root, group, tracer, patient, slice_id)
            group_records.append(record)
            all_records.append(record)
        make_plate(group_records, args.output_dir / f"{group}.png", f"MMRAD {group} lesion fixed cases")
    make_plate(all_records, args.output_dir / "all_fixed_lesion_cases.png", "MMRAD fixed lesion cases")
