...
class DualModalNetworkFeatureAggregator(NetworkFeatureAggregator):
    """Feature aggregator for dual-modal input (e.g. PET + CT).

    Runs the shared backbone on each modality independently and
    concatenates the intermediate feature maps channel-wise.  The
    resulting feature dimension at every extracted layer is therefore
    twice the single-modal dimension.

    Usage:
        aggregator = DualModalNetworkFeatureAggregator(backbone, layers, device)
        features = aggregator((pet_batch, ct_batch))   # tuple input
    """

    def forward(self, images):
        """Extract and merge features from two image modalities.

        Args:
            images: tuple (pet_images, ct_images), each [B, 3, H, W]
        Returns:
            dict  layer_name -> concatenated feature map [B, 2*C, H', W']
        """
        pet_images, ct_images = images

        # --- PET forward pass ---
        self.outputs.clear()
        with torch.no_grad():
            try:
                _ = self.backbone(pet_images)
            except LastLayerToExtractReachedException:
                pass
        pet_features = {k: v.clone() for k, v in self.outputs.items()}

        # --- CT forward pass ---
        self.outputs.clear()
        with torch.no_grad():
            try:
                _ = self.backbone(ct_images)
            except LastLayerToExtractReachedException:
                pass
        ct_features = {k: v.clone() for k, v in self.outputs.items()}

        # Concatenate channel-wise at each layer
        merged = {}
        for layer in self.layers_to_extract_from:
            merged[layer] = torch.cat([pet_features[layer], ct_features[layer]], dim=1)
        return merged

    def feature_dimensions(self, input_shape):
        """Feature dimensions are doubled (PET channels + CT channels)."""
        _input = torch.ones([1] + list(input_shape)).to(self.device)
        self.outputs.clear()
        with torch.no_grad():
            try:
                _ = self.backbone(_input)
            except LastLayerToExtractReachedException:
                pass
        single_dims = [self.outputs[layer].shape[1]
                       for layer in self.layers_to_extract_from]
        return [d * 2 for d in single_dims]
