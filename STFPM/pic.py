...
def save_heatmap(
    ...
    percentile: float = 98.0,             # 当使用分位数或回退时使用
    max_area_ratio: float = 0.5,          # 若前景面积超过脑区比例，则自动提高阈值
    min_region: int = 30                  # 去除很小连通域
):
    """
    ...
    if brain_area > 0 and fg_area > max_area_ratio * brain_area:
        vals = heatmap_masked[brain_mask]
        thr2 = float(np.percentile(vals, max(99.0, percentile)))  # 自动提高阈值
    ...
    try:
        if morphology is not None:
            ...
            binary_map = morphology.binary_closing(binary_map, morphology.disk(2)).astype(np.uint8)
    except Exception:
        pass
