...
def _pixel_slice_bootstrap(labels, masks, maps, iters, seed, exact_auroc=None, exact_aupr=None, progress=False):
    ...
    if exact_auroc is None or exact_aupr is None:
        ...
        if exact_auroc is None:
            exact_auroc = _safe_auroc(y_true, y_score)
        if exact_aupr is None:
            exact_aupr = _safe_ap(y_true, y_score)
    ...
    aurocs, auprs = [], []
    n = len(abn_idx)
    for boot_idx in range(int(iters)):
        ...
        aurocs.append(auroc)
        auprs.append(aupr)
        if progress and ((boot_idx + 1) % 50 == 0 or boot_idx + 1 == int(iters)):
            print(f"Pixel histogram slice bootstrap: {boot_idx + 1}/{iters}", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="Compute STFPM bootstrap metrics from an eval cache.")
    ...
    print(f"Loading eval cache from {args.cache}", flush=True)
    ...
    print(
        ...
        f"maps={maps.shape}, bootstrap_iters={args.bootstrap_iters}",
        flush=True,
    )
    ...
    if args.exact_pixel_auroc is None or args.exact_pixel_aupr is None:
    ...
    px_auroc, px_aupr, binned = _pixel_slice_bootstrap(
        ...
        args.seed + 10,
        args.exact_pixel_auroc,
        args.exact_pixel_aupr,
        progress=True,
    )
