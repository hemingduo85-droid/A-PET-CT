...
"""
Run example:
cd /data/cyf/codes/A-PET-CT/AE_Flow_successful
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "AE-FLOW")
...
def build_input(paths, modalities, image_size):
    ...
    fused = np.stack(channels[:3], axis=0).astype(np.float32)
    fused = (fused - 0.5) / 0.5
    return torch.from_numpy(fused).unsqueeze(0), images


def load_model_for_tracer(args, tracer, modalities, device):
    ns = argparse.Namespace(checkpoint=args.psma_checkpoint if tracer == "PSMA" else args.fdg_checkpoint, checkpoint_root=args.checkpoint_root, tracer=tracer, subnet=args.subnet)
    ckpt = find_checkpoint(ns, modalities)
...
def render_case(args, model, ckpt_path, modalities, device, data_root, group, tracer, patient, slice_id):
    ...
    check_paths(paths)
    image, images = build_input(paths, modalities, args.image_size)
    image = image.to(device)
    with torch.no_grad():
        rec, _, _ = model(image)
        raw_map = torch.abs(image - rec).mean(dim=1).squeeze(0).detach().cpu().numpy().astype(np.float32)
    ...
    pet_img = colorize(images["pet"], "magma")
    ct_img = gray_image(images["ct"])
    panels = {
        "PET": pet_img,
        "CT": ct_img,
        "GT": mask_image(mask),
        "Pred": mask_image(pred, color=(255, 128, 0)),
        "AE-FLOW": colorize(raw_map, "jet"),
    }
    ...
    save_image(case_dir / "AE-FLOW_heatmap.png", panels["AE-FLOW"], args.image_size)
    meta = {"method": METHOD, "group": group, "tracer": tracer, "patient": patient, "slice": slice_id, "score": float(np.max(raw_map)), "score_mode": "max pixel score", "threshold": threshold, "best_f1": best_f1, "checkpoint": str(ckpt_path), "case_dir": str(case_dir)}
...
def main():
    ...
    for group in GROUPS:
        for tracer in TRACERS:
            ...
            model, ckpt = models[tracer]
            records_by_group[group].append(render_case(args, model, ckpt, modalities, device, data_root, group, tracer, patient, slice_id))
