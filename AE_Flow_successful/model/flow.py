...
class NormalizingFlow(nn.Module):
    def __init__(self, subnet="resnet_type", flow_step=8):
        ...
        for k in range(flow_step):
            if (subnet == "conv_type"):
                self.inn.append(AllInOneBlock, subnet_constructor=subnet_conv, permute_soft=True)
            elif (subnet == "resnet_type"):
                # raise NotImplementedError
                self.inn.append(AllInOneBlock, subnet_constructor=subnet_restnet, permute_soft=True)
            else:
                raise NotImplementedError
