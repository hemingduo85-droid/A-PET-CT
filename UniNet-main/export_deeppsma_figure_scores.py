#!/usr/bin/env python3
"""DeepPSMA generalization commands (UniNet):
python export_deeppsma_figure_scores.py --source_dataset PSMA --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --modality petct --gpu 4 --epochs 30 --output_dir generalization_outputs/PSMA_weights/figure_scores --skip_existing --skip_require_pixel

python export_deeppsma_figure_scores.py --source_dataset FDG --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --modality petct --gpu 6 --epochs 30 --output_dir generalization_outputs/FDG_weights/figure_scores --skip_existing --skip_require_pixel
"""
# Run examples:
#   python export_deeppsma_figure_scores.py --dataset_name PSMA --modality petct --gpu 4 --epochs 30 --skip_existing --skip_require_pixel
#   python export_deeppsma_figure_scores.py --dataset_name FDG --modality petct --gpu 6 --epochs 30 --skip_existing --skip_require_pixel
...
def collect_scores(args):
    ...
    import main_petct
    from datasets_petct import PETCTDataset
    ...
    for idx, (label, score, path, mask) in enumerate(zip(labels, scores, paths, masks)):
        ...
        pix = pixel_metric_row(args.method, "DeepPSMA", args.modality, case_id, slice_id, path, anomaly_map[idx], mask)
        if pix is not None:
