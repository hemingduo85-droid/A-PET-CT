...
def save_weights(modules_list: list, ckpt_path, suffix, device=None):
...
def load_weights(modules_list: list, ckpt_path, suffix, device=None):
    print("Loading weights from {}".format(join(ckpt_path, f"{suffix}.pth")), flush=True)

    map_location = torch.device(device) if device is not None else None
    state_dict = torch.load(join(ckpt_path, f"{suffix}.pth"), map_location=map_location)
    print("weight file has been loaded", flush=True)
    new_state = {"tt": None,
                 "bn": None,
                 "st": None,
                 "dfs": None
