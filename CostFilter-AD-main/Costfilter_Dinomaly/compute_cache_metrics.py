...
def _bootstrap_ci(y_true, y_score, metric_fn, iters, seed):
    ...
    return value, (float(np.clip(low, 0.0, 1.0)), float(np.clip(high, 0.0, 1.0)))
...
def _pixel_slice_bootstrap(labels, masks, maps, iters, seed, bins):
    ...
    flat_score = np.nan_to_num(flat_score, nan=0.0, posinf=0.0, neginf=0.0)
    auroc_value = _safe_auroc(flat_y, flat_score)
    aupr_value = _safe_ap(flat_y, flat_score)
    if iters <= 0:
    ...
    for row, idx in enumerate(abn_idx):
        scores = np.nan_to_num(maps[idx].reshape(-1), nan=0.0, posinf=0.0, neginf=0.0)
    ...
    for i in range(iters):
        ...
        if (i + 1) % 50 == 0 or (i + 1) == iters:
            print(f"Pixel histogram slice bootstrap: {i + 1}/{iters}")
...
def main():
    parser = argparse.ArgumentParser(description="Compute CostFilter-AD bootstrap metrics from an eval cache.")
    ...
    print(f"Loading eval cache from {args.cache}")
    ...
    print(
        ...
        f"bootstrap_iters={args.bootstrap_iters}"
    )
    ...
    image_line = "[Slice-Img]  " + "  ".join([_fmt("AUROC", img_auroc), _fmt("AP", img_ap), _fmt("F1", img_f1)])
    print(image_line)

    print("Computing pixel-level Slice-Px(abn) metrics and 95CI...")
    px_auroc, px_aupr = _pixel_slice_bootstrap(labels, masks, maps, args.bootstrap_iters, args.seed + 10, args.hist_bins)
    pixel_line = "[Slice-Px(abn)]  " + "  ".join([_fmt("AUROC", px_auroc), _fmt("AUPR", px_aupr)])
    print(pixel_line)

    print("Computing patient-level metrics and 95CI...")
    ...
    patient_line = (
        ...
        + "  ".join([_fmt("AUROC", pat_auroc), _fmt("AP", pat_ap), _fmt("F1", pat_f1)])
    )

    lines = [image_line, pixel_line, patient_line]
    text = "\n".join(lines) + "\n"
    print(text, end="")
    if args.output:
