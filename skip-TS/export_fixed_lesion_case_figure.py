...
"""
Run example:
cd /data/cyf/codes/A-PET-CT/skip-TS
...
from export_figure_scores import default_checkpoint_path, resolve_save_path
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "Skip-TS")
...
def checkpoint_for_tracer(args, tracer: str, input_mode: str) -> Path:
    explicit = args.psma_checkpoint if tracer == "PSMA" else args.fdg_checkpoint
    if explicit:
        return resolve_path(explicit)
    dataset = tracer.lower()
    save_path = resolve_save_path(args.checkpoint_root, dataset, args.modalities, input_mode)
    return resolve_path(default_checkpoint_path(save_path, args.net, dataset, args.modalities, input_mode, args.epochs, args.seed))


def load_models_for_tracer(args, tracer: str, input_mode: str, device):
    ckpt_path = checkpoint_for_tracer(args, tracer, input_mode)
    if not ckpt_path.exists():
        raise FileNotFoundError(f"Checkpoint not found for {tracer}: {ckpt_path}")
    encoder, decoder = _build_models(args.modalities, args.replicate_channels, input_mode, device)
    state = torch.load(str(ckpt_path), map_location=device)
    decoder.load_state_dict(state, strict=True)
    encoder.eval()
    decoder.eval()
    print(f"Loaded {tracer} checkpoint: {ckpt_path}")
    return encoder, decoder, ckpt_path


def build_input(paths, transform, mean, std):
    modality_tensors = {
        "ct": transform(Image.open(paths["ct"]).convert("L")),
        "pet": transform(Image.open(paths["pet"]).convert("L")),
    }
    image = torch.cat([modality_tensors["ct"], modality_tensors["pet"], modality_tensors["pet"]], dim=0)
    mean_t = torch.tensor(mean).view(-1, 1, 1)
    std_t = torch.tensor(std).view(-1, 1, 1)
    return ((image - mean_t) / std_t).unsqueeze(0)


def render_case(args, encoder, decoder, ckpt_path, transform, mean, std, device, data_root, group, tracer, patient, slice_id):
    paths = case_paths(data_root, tracer, patient, slice_id)
    check_paths(paths)
    image = build_input(paths, transform, mean, std).to(device)
    with torch.no_grad():
        inputs = encoder(image)
        outputs = decoder(inputs[3], inputs[0:3], args.res)
        raw_map, _ = cal_anomaly_map(inputs[0:3], outputs, image.shape[-1], amap_mode="a")
    ...
    save_image(case_dir / "Skip-TS_heatmap.png", heatmap_img, args.image_size)
    ...
    return {"meta": meta, "panels": {"PET": pet_img, "CT": ct_img, "GT": gt_img, "Pred": pred_img, "Skip-TS": heatmap_img}}
...
def draw_text(draw, xy, text, font, fill=(0, 0, 0), anchor=None):
    if anchor:
...
def make_plate(records, save_path: Path, title: str):
    ...
    meta_font = load_font(13, bold=False)
    draw_text(draw, (width // 2, 12), title, title_font, anchor="ma")
    y = top
    for col, name in enumerate(PANEL_NAMES):
        x = left + col * (tile + gap)
        draw_text(draw, (x + 4, y), name, header_font)
    y += header_h
    for row_idx, record in enumerate(records, 1):
        ...
        draw_text(draw, (left, y + 4), meta_text, meta_font)
        y += meta_h
        for col, name in enumerate(PANEL_NAMES):
            x = left + col * (tile + gap)
...
def main():
    args = parse_args()
    device = _device(args.device if args.device.startswith("cuda:") or args.device == "cpu" else f"cuda:{args.cuda}")
    args.output_dir = resolve_path(args.output_dir)
    data_root = resolve_path(args.data_root)
    input_mode = args.input_mode if len(args.modalities) > 1 else "single"
    if input_mode != "pseudo_rgb":
        raise ValueError("This fixed-case script expects --input_mode pseudo_rgb for ct/pet.")
    transform, _gt_transform, mean, std = get_data_transforms(
        args.image_size,
        args.image_size,
    ...
    models = {tracer: load_models_for_tracer(args, tracer, input_mode, device) for tracer in TRACERS}
    records_by_group = {group: [] for group in GROUPS}
    for group in GROUPS:
        for tracer in TRACERS:
            patient, slice_id = FIXED_CASES[group][tracer]
            encoder, decoder, ckpt_path = models[tracer]
            records_by_group[group].append(
                render_case(args, encoder, decoder, ckpt_path, transform, mean, std, device, data_root, group, tracer, patient, slice_id)
            )
