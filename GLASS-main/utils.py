...
def create_storage_folder(
        main_folder_path, project_folder, group_folder, run_name, mode="iterate", save_dir=None
):
    """
    解析实验输出根目录（权重、results.csv 等）。

    优先级：save_dir > main_folder_path/project/group/run_name
    """
    if save_dir:
    ...
    parts = [main_folder_path]
    if project_folder and project_folder != "project":
        parts.append(project_folder)
    if group_folder and group_folder != "group":
        parts.append(group_folder)
    if run_name and run_name != "test":
