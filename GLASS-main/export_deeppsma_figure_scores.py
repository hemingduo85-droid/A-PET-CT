#!/usr/bin/env python3
"""DeepPSMA generalization commands (GLASS):
python export_deeppsma_figure_scores.py --source_dataset PSMA --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --modality petct --gpu 4 --output_dir generalization_outputs/PSMA_weights/figure_scores --skip_existing --skip_require_pixel

python export_deeppsma_figure_scores.py --source_dataset FDG --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1 --modality petct --gpu 4 --output_dir generalization_outputs/FDG_weights/figure_scores --skip_existing --skip_require_pixel
"""
# Run examples:
#   python export_deeppsma_figure_scores.py --dataset psma --modality petct --gpu 4 --skip_existing --skip_require_pixel
#   python export_deeppsma_figure_scores.py --dataset fdg --modality petct --gpu 4 --skip_existing --skip_require_pixel
...
def main():
    ...
    for idx, (score, label, path, segmentation, mask_gt) in enumerate(zip(scores, labels, paths, segmentations, masks_gt)):
        ...
        pix = pixel_metric_row("GLASS", "DeepPSMA", args.modality, idx, pid, label, path, segmentation, mask_gt)
        if pix is not None:
