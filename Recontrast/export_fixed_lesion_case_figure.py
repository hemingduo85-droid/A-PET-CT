...
python export_fixed_lesion_case_figure.py \
  ...
  --image_size 256 \
  --crop_size 224 \
  --psma_save_name recontrast_psma_petct \
  --fdg_save_name recontrast_fdg_petct \
  --output_dir fixed_lesion_case_figures
"""
"""Render fixed PSMA/FDG lesion-size cases for ReContrast comparison figures."""
...
from export_figure_scores import build_model
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "ReContrast")
...
def center_crop_array(arr, size):
    arr = np.asarray(arr)
    h, w = arr.shape[:2]
    top = max(0, (h - size) // 2)
    left = max(0, (w - size) // 2)
...
def read_gray(path: Path, image_size: int, crop_size: int, is_mask: bool = False):
    ...
    arr = center_crop_array(arr, crop_size)
    if is_mask:
...
def top_ratio_score(score_map, ratio):
...
def checkpoint_for_tracer(args, tracer: str) -> Path:
    ...
    save_name = args.psma_save_name if tracer == "PSMA" else args.fdg_save_name
    if not save_name:
        save_name = f"recontrast_{dataset}_{args.modality.lower()}"
    return resolve_path(args.save_dir) / save_name / f"epoch{args.epochs}_{dataset}_{args.modality.lower()}.pth"
...
def load_model_for_tracer(args, tracer: str, device):
    ...
    model_args = SimpleNamespace(replicate_channels=args.replicate_channels)
...
def build_input(paths: dict[str, Path], modality: str, transform):
    pet = Image.open(paths["pet"]).convert("L")
    if modality == "petct":
        ct = Image.open(paths["ct"]).convert("L")
        image = Image.merge("RGB", (ct, pet, pet))
    elif modality == "pet":
        image = pet.convert("RGB")
    elif modality == "ct":
        image = Image.open(paths["ct"]).convert("L").convert("RGB")
    else:
        raise ValueError("modality must be pet, ct, or petct")
    return transform(image).unsqueeze(0)


def render_case(args, model, ckpt_path: Path, gaussian_kernel, transform, device, data_root: Path,
                group: str, tracer: str, patient: str, slice_id: str):
    paths = case_paths(data_root, tracer, patient, slice_id)
    check_paths(paths)

    image = build_input(paths, args.modality.lower(), transform).to(device)
    with torch.no_grad():
        en2, de2 = model(image)
        anomaly_map, _ = cal_anomaly_maps(en2, de2, image.shape[-1])
        if args.resize_mask is not None and int(args.resize_mask) != image.shape[-1]:
            anomaly_map = F.interpolate(
                anomaly_map,
                size=(int(args.resize_mask), int(args.resize_mask)),
                mode="bilinear",
                align_corners=False,
            )
        anomaly_map = gaussian_kernel(anomaly_map)
        anomaly_map = torch.nan_to_num(anomaly_map, nan=0.0, posinf=0.0, neginf=0.0)
    raw_map = anomaly_map[0, 0].detach().cpu().numpy().astype(np.float32)

    display_size = int(raw_map.shape[-1])
    pet_arr = read_gray(paths["pet"], args.image_size, args.crop_size)
    ct_arr = read_gray(paths["ct"], args.image_size, args.crop_size)
    mask = read_gray(paths["mask"], args.image_size, args.crop_size, is_mask=True)
    if mask.shape != raw_map.shape:
        ...
        pet_arr = np.asarray(Image.fromarray((normalize01(pet_arr) * 255).astype(np.uint8)).resize((display_size, display_size))) / 255.0
        ct_arr = np.asarray(Image.fromarray((normalize01(ct_arr) * 255).astype(np.uint8)).resize((display_size, display_size))) / 255.0

    threshold, best_f1 = best_f1_threshold(mask, raw_map)
    pred = (raw_map >= threshold).astype(np.uint8)
    score = top_ratio_score(raw_map, args.max_ratio)
    ...
    gt_img = mask_image(mask, color=(0, 255, 80))
    ...
    save_image(case_dir / "ReContrast_heatmap.png", heatmap_img, args.image_size)
    ...
    meta = {
        ...
        "slice": slice_id,
        "score": score,
        "score_mode": f"top {args.max_ratio:.4f} pixel ratio mean",
        "threshold": threshold,
        "best_f1": best_f1,
...
def parse_args():
    ...
    parser.add_argument("--psma_save_name", default="recontrast_psma_petct")
    parser.add_argument("--fdg_save_name", default="recontrast_fdg_petct")
    ...
    parser.add_argument("--resize_mask", type=int, default=None)
...
def main():
    ...
    cuda_config = str(args.cuda).lower()
    ...
    transform, _gt_transform = get_data_transforms(
        size=args.image_size,
        isize=args.crop_size,
        num_channels=args.replicate_channels,
        normalize=True,
    )
    gaussian_kernel = get_gaussian_kernel(kernel_size=5, sigma=4).to(device)
    models = {}
    for tracer in TRACERS:
        models[tracer] = load_model_for_tracer(args, tracer, device)
    ...
    for group in GROUPS:
        for tracer in TRACERS:
            ...
            records_by_group[group].append(
                render_case(
                    args, model, ckpt_path, gaussian_kernel, transform, device,
                    data_root, group, tracer, patient, slice_id,
                )
            )
