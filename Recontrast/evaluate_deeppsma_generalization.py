#!/usr/bin/env python3
"""
DeepPSMA1 direct generalization test for ReContrast.

Data root:
  /data/cyf/shared_data/A-PETCT/2d_equal_mask50/deeppsma1

Run with existing PSMA weights:
  python evaluate_deeppsma_generalization.py --source_tracer PSMA --modality petct --gpu 2 > deeppsma_psma.log 2>&1 &

Run with existing FDG weights:
  python evaluate_deeppsma_generalization.py --source_tracer FDG --modality petct --gpu 0 > deeppsma_fdg.log 2>&1 &
"""
...
class DeepPSMAFlatDataset(Dataset):
    def __init__(self, data_root, modality="petct", image_size=256, crop_size=224, replicate_channels=3):
        self.root = Path(data_root).expanduser().resolve()
        self.modality = modality.lower()
        self.replicate_channels = int(replicate_channels)
        self.transform, self.gt_transform = get_data_transforms(
            size=image_size,
            isize=crop_size,
    ...
    def __getitem__(self, idx):
        ...
        if self.modality == "petct":
            ...
            image = gray.convert("RGB") if self.replicate_channels == 3 else gray
        ...
        if label and mask_path is not None:
            ...
            mask = torch.zeros((1, img.shape[-2], img.shape[-1]), dtype=torch.float32)
        return img, mask, label, sample_id
...
def evaluate(model, loader, device, args):
    model.eval()
    gaussian_kernel = rc.get_gaussian_kernel(kernel_size=5, sigma=4).to(device)
    ...
    for img, gt, label, paths in tqdm(loader, desc="DeepPSMA eval", leave=True):
        ...
        anomaly_map, _ = rc.cal_anomaly_maps(en, de, img.shape[-1])
        if args.resize_mask:
            anomaly_map = F.interpolate(anomaly_map, size=(args.resize_mask, args.resize_mask), mode="bilinear", align_corners=False)
            gt = F.interpolate(gt, size=(args.resize_mask, args.resize_mask), mode="nearest")
        anomaly_map = torch.nan_to_num(gaussian_kernel(anomaly_map), nan=0.0, posinf=0.0, neginf=0.0)
...
def build_parser():
    parser = argparse.ArgumentParser(description="ReContrast direct DeepPSMA1 generalization evaluator")
...
def main():
    ...
    args.dataset = source
    args.replicate_channels = 3
    args.save_name = f"recontrast_{source}_{args.modality}"
    ...
    rc.args = SimpleNamespace(replicate_channels=args.replicate_channels)
    rc.device = device

    dataset = DeepPSMAFlatDataset(args.data_root, args.modality, args.image_size, args.crop_size, args.replicate_channels)
    print(f"DeepPSMA root: {Path(args.data_root).expanduser().resolve()}")
    print(f"Source weights: {args.source_tracer.upper()}")
    ...
    checkpoint = args.checkpoint or str(REPO_DIR / "results" / args.save_name / f"epoch{args.epochs}_{source}_{args.modality}.pth")
    ...
    out_path = out_dir / f"{args.source_tracer.upper()}_{args.modality}_slice_only_metrics.txt"
