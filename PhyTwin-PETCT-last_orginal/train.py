...
说明：
    ...
    Adaptive PHYSIO prior 和图像级正常分布校准。主方法默认不使用 residual memory
    bank（residual_weight=1.0, memory_weight=0.0）；memory bank 仅保留为消融项。
...
三、模块消融实验命令
消融依据：
    为了减少训练随机性对模块贡献判断的影响，下面的后处理/校准类消融均固定同一个
    checkpoint，只通过 --load_ckpt 改变推理阶段组件。若要做训练组件消融，
    例如去掉 uncertainty loss，则需要重新训练，并在表格中单独标注。

    M1. CT 条件正常代谢表征学习
    说明：M1 输出 P_N，是后续 residual 和 PHYSIO 的基础，不能像后处理一样
    直接关闭。它的贡献用 residual-only 基础实验体现：只保留 P 与 P_N 的残差响应，
    ...
    python train.py \
      ...
      --memory_weight 0.0 \
      --gpu 5 \
      --save_dir ./ablation_psma_m1_residual_only

    M2. 可选结构消融：+ residual memory bank
    作用：验证 residual memory bank 是否带来额外收益。当前 PSMA 实验显示该模块没有稳定
    增益，因此主方法默认不使用 memory bank；该命令仅用于论文消融说明。
    python train.py \
      --data_root /data/cyf/shared_data/PET-CT/AutoPET/2d_equal_mask50/PSMA \
      --load_ckpt \
      --experiment_name PhyTwin_PETCT_PSMA \
      ...
      --memory_weight 0.55 \
      --gpu 5 \
      --save_dir ./ablation_psma_m2_residual_memory

    M3. + Adaptive PHYSIO：病灶保护的自适应生理摄取抑制
    作用：在 M1 的 uncertainty residual map 基础上加入完整生理摄取校正。群体生理先验、逐图器官样
    ...
    python train.py \
      ...
      --adaptive_lesion_protect 0.70 \
      --gpu 5 \
      --save_dir ./ablation_psma_m3_adaptive_physio

    M4. + 多尺度病灶感知推理
    作用：在 M1+M3 的校正异常图基础上，将 image-level 推理从 top-1% 平均改为
    ...
    训练组件消融：去掉 uncertainty loss，需要重新训练
    作用：验证 Normal Twin 中不确定性建模是否有助于稳定正常代谢表征。
    nohup python train.py \
      --data_root /data/cyf/shared_data/PET-CT/AutoPET/2d_equal_mask50/PSMA \
      --epochs 30 \
      --batch_size 8 \
      ...
      --no_uncertainty \
      --nll_weight 0 \
      --experiment_name PhyTwin_PETCT_PSMA_no_uncertainty \
      --gpu 5 \
      --save_dir ./ablation_psma_no_uncertainty \
      > ablation_psma_no_uncertainty.log 2>&1 &
