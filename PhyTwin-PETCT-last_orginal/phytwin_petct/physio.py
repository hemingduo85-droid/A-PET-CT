...
class AdaptivePhysioSuppressor:
    ...
    def __init__(
        ...
        self.pet_weight = float(pet_weight)
    ...
    def _component_mask(self, anomaly_map, pet_map, ct_map=None, static_prior=None):
        ...
        pet_threshold = float(np.quantile(pet.reshape(-1), self.pet_quantile))
        anomaly_threshold = float(np.quantile(anomaly.reshape(-1), self.anomaly_quantile))
        pet_hot = pet >= pet_threshold
        anomaly_hot = anomaly >= anomaly_threshold
        if prior is not None:
            prior_gate = prior >= max(0.10, float(np.quantile(prior.reshape(-1), 0.70)))
            candidate = pet_hot | (anomaly_hot & (prior_gate | (pet >= np.quantile(pet.reshape(-1), 0.90))))
        else:
            candidate = pet_hot | (anomaly_hot & (pet >= np.quantile(pet.reshape(-1), 0.90)))
        ...
        for i in range(1, num + 1):
            ...
            # Small and PET-dominant CT/PET-mismatched components are more
            # likely to be lesions, so suppress them less. The anomaly peak is
            # intentionally not used as lesion evidence, because false-positive
            # physiological hotspots are also high-anomaly by definition.
            ...
            physio_like = (
                ...
                + self.pet_weight * pet_hot_score
                + self.anomaly_weight * anomaly_peak
            )
            physio_like = physio_like / max(
                1e-8,
                self.large_weight + self.prior_weight + self.ct_weight + self.pet_weight + self.anomaly_weight,
            )
