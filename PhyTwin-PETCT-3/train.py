...
服务器完整训练命令：
    ...
    nohup python train.py \
      ...
      --image_size 256 \
      --gpu 5 \
      --save_dir ./saved_results_phytwin_psma_3407 \
      --seed 3407 \
      > phytwin_psma_3407.log 2>&1 &
...
如果旧 checkpoint 里没有多尺度 lesion_z 校准，脚本会自动用 train/normal 重算校准参数，不会重训 Normal Twin 或重建 memory。
...
from phytwin_petct.physio import PhysiologicalUptakePrior, pet_tensor_to_gray, save_prior_image
from phytwin_petct.scoring import (fuse_maps, lesion_component_score, lesion_likeness_score,
                                   normal_calibrated_score, positive_zscore_map, predict_mask,
                                   topk_score)
...
def parse_args():
    ...
    args.method_name = "PhyTwin_PETCT"
    return args
...
def compute_lesion_score(final, pet_map, ct_map, prior, args):
...
def calibrate_normal_lesion_scores(model, memory, prior, loader, device, args, calibration, logger=None):
    ...
    for _idx, batch in limited(loader, args.max_memory_batches):
        ...
        for b in range(residual.shape[0]):
            ...
            scores.append(compute_lesion_score(final, pet_gray, ct_gray, prior, args))
...
def build_memory_and_prior(model, loader, device, args, logger):
    ...
    for i in range(residuals.shape[0]):
        ...
        normal_lesion_scores.append(compute_lesion_score(
            final,
            pet_maps[i].numpy().astype(np.float32),
            ct_maps[i].numpy().astype(np.float32),
            prior,
            args,
        ))
...
def evaluate(model, memory, prior, dynamic_suppressor, calibration, loader, device, args, logger, vis_dir):
    ...
    for _idx, batch in limited(loader, args.max_test_batches):
        ...
        if args.score_mode == "component":
            ...
            raw_score = compute_lesion_score(final, pet_gray, ct_gray, prior, args)
            image_score = (raw_score - calibration.get("normal_lesion_ms_mean", calibration.get("normal_lesion_mean", calibration["normal_image_mean"]))) / (
                calibration.get("normal_lesion_ms_std", calibration.get("normal_lesion_std", calibration["normal_image_std"])) + 1e-8
            )
        else:
...
def main():
    ...
    ckpt_dir = os.path.join("./ckpts", args.method_name)
    ...
    if args.load_ckpt:
        ...
        if args.score_mode == "lesion_z" and "normal_lesion_ms_mean" not in calibration:
            logger.info("Checkpoint lacks multi-scale lesion_z calibration; recalibrating on train/normal without retraining.")
