...
class PetCTDataset(torch.utils.data.Dataset):
    """PET-CT anomaly detection dataset.

    Supports five input modes:
      - 'ct':          grayscale CT → [CT, CT, CT] pseudo-RGB  (3ch)
      - 'pet':         grayscale PET → [PET, PET, PET] pseudo-RGB  (3ch)
      - 'dual':        [CT, PET, PET] pseudo-RGB  (3ch, early pixel fusion)
      - 'dual_rgbd':   [CT,CT,CT] cat [PET,PET,PET] → 6ch  (feature-level fusion via DinomalyRGBD)
      - 'dual_mulsen': returns (ct_3ch, pet_3ch) separately  (score-level fusion, MulSen style)
    """

    VALID_MODES = ('ct', 'pet', 'dual', 'dual_rgbd', 'dual_mulsen')

    def __init__(self, data_root, transform, gt_transform, phase, input_mode='dual'):
        assert phase in ('train', 'test')
        assert input_mode in self.VALID_MODES, f"input_mode must be one of {self.VALID_MODES}"
        ...
        self.img_paths = []  # list of (ct_path, pet_path)
        self.gt_paths = []
        self.labels = []
        self.types = []
        self._load_samples()

        self.img_paths = np.array(self.img_paths, dtype=object)
        self.gt_paths = np.array(self.gt_paths, dtype=object)
        self.labels = np.array(self.labels)
        self.types = np.array(self.types)

    def _add_patient_dir(self, base_dir, label):
        if not os.path.exists(base_dir):
            return
        for patient_name in sorted(os.listdir(base_dir)):
            ...
            for fname in sorted(os.listdir(ct_dir)):
                ...
                if not os.path.exists(pet_path):
                    continue
                mask_path = None
                if label == 1 and os.path.exists(label_dir):
                    candidate = os.path.join(label_dir, fname)
                    if os.path.exists(candidate):
                        mask_path = candidate
                self.img_paths.append((ct_path, pet_path))
                self.gt_paths.append(mask_path)
                self.labels.append(label)
                self.types.append('good' if label == 0 else 'anomaly')
    ...
    def _get_gt(self, mask_path, label, ref_img):
        if label == 0 or mask_path is None:
            return torch.zeros([1, ref_img.size()[-2], ref_img.size()[-1]])
        gt = Image.open(mask_path).convert('L')
        gt = self.gt_transform(gt)
        return (gt > 0).float()

    def __getitem__(self, idx):
        ct_path, pet_path = self.img_paths[idx]
        ...
        if self.input_mode == 'ct':
            img = Image.merge('RGB', (ct_gray, ct_gray, ct_gray))
            img = self.transform(img)
            if self.phase == 'train':
                return img, label
            return img, self._get_gt(mask_path, label, img), label, str(ct_path)

        elif self.input_mode == 'pet':
            img = Image.merge('RGB', (pet_gray, pet_gray, pet_gray))
            img = self.transform(img)
            if self.phase == 'train':
                return img, label
            return img, self._get_gt(mask_path, label, img), label, str(ct_path)
            ...
            img = Image.merge('RGB', (ct_gray, pet_gray, pet_gray))
            img = self.transform(img)
            if self.phase == 'train':
                return img, label
            return img, self._get_gt(mask_path, label, img), label, str(ct_path)

        elif self.input_mode == 'dual_rgbd':
            ct_rgb = Image.merge('RGB', (ct_gray, ct_gray, ct_gray))
            pet_rgb = Image.merge('RGB', (pet_gray, pet_gray, pet_gray))
            ct_t = self.transform(ct_rgb)   # [3,H,W]
            pet_t = self.transform(pet_rgb) # [3,H,W]
            img = torch.cat([ct_t, pet_t], dim=0)  # [6,H,W]
            if self.phase == 'train':
                return img, label
            return img, self._get_gt(mask_path, label, ct_t), label, str(ct_path)

        else:  # dual_mulsen
            ct_rgb = Image.merge('RGB', (ct_gray, ct_gray, ct_gray))
            pet_rgb = Image.merge('RGB', (pet_gray, pet_gray, pet_gray))
            ct_t = self.transform(ct_rgb)
            pet_t = self.transform(pet_rgb)
            if self.phase == 'train':
                return (ct_t, pet_t), label
            return (ct_t, pet_t), self._get_gt(mask_path, label, ct_t), label, str(ct_path)
