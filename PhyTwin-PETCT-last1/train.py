...
实验命名规则：
    默认会按数据集文件夹名保存 checkpoint，避免 PSMA 和 FDG 互相覆盖。
    例如 PSMA -> ./ckpts/PhyTwin_PETCT_PSMA，
    FDG -> ./ckpts/PhyTwin_PETCT_FDG。
    如果需要固定 checkpoint 名称，可以手动指定 --experiment_name。

一、主实验：完整训练命令
说明：
    主实验训练 30 epochs，使用 Normal Twin 构建不确定性感知 residual map、
    Adaptive PHYSIO prior 和图像级正常分布校准。主方法使用 residual map
    （residual_weight=1.0, memory_weight=0.0），不使用 residual memory bank。
    训练阶段仍只使用 train/normal。
    ...
    nohup python train.py \
      ...
      --gpu 5 \
      --save_dir ./saved_results_psma_main \
      > phytwin_psma_main.log 2>&1 &
    ...
    nohup python train.py \
      ...
      --adaptive_lesion_protect 0.70 \
      --gpu 5 \
      --save_dir ./saved_results_fdg_main \
      > phytwin_fdg_main.log 2>&1 &

二、主实验：只测试/重新生成可视化
说明：
    --load_ckpt 会加载已有 checkpoint。若 checkpoint 中缺少当前 scoring 的校准参数，
    脚本会仅用 train/normal 重新计算校准统计量，不重训 Normal Twin。
    ...
    python train.py \
      ...
      --adaptive_lesion_protect 0.70 \
      --gpu 5 \
      --save_dir ./saved_results_psma_eval
    ...
    python train.py \
      ...
      --adaptive_lesion_protect 0.70 \
      --gpu 5 \
      --save_dir ./saved_results_fdg_eval

三、模块消融实验命令
消融依据：
    四组消融按模块逐步累加。M1 加载已有的 no-uncertainty 独立 checkpoint；
    M2-M4 固定加载同一个带 uncertainty 的主模型 checkpoint，仅改变推理阶段组件，
    从而避免训练随机性干扰 Adaptive PHYSIO 和多尺度病灶感知推理的比较。
    四组实验均不使用 residual memory bank。

    M1. no uncertainty residual-only
    作用：关闭 Normal Twin uncertainty head 和 uncertainty loss，只保留
    P 与 P_N 的普通绝对残差响应；不使用 Adaptive PHYSIO 和多尺度病灶感知推理。
    加载已有的 ./ckpts/PhyTwin_PETCT_PSMA_no_uncertainty_residual/BEST_PHYTWIN.pth。
    python train.py \
      --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/FDG \
      --load_ckpt \
      --experiment_name PhyTwin_PETCT_FDG_no_uncertainty_residual \
      --score_mode top1pct \
      --no_physio \
      --no_uncertainty \
      --residual_weight 1.0 \
      --memory_weight 0.0 \
      --gpu 5 \
      --save_dir ./ablation_fdg_m1_no_uncertainty_residual_only

    M2. uncertainty residual-only
    作用：启用 Normal Twin uncertainty head，用预测不确定性校正 residual map；
    不使用 Adaptive PHYSIO 和多尺度病灶感知推理。
    python train.py \
      --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/FDG \
      --load_ckpt \
      --experiment_name PhyTwin_PETCT_FDG \
      ...
      --memory_weight 0.0 \
      --gpu 4 \
      --save_dir ./ablation_fdg_m2_uncertainty_residual_only

    M3. + Adaptive PHYSIO：病灶保护的自适应生理摄取抑制
    作用：在 M2 的 uncertainty residual map 基础上加入完整生理摄取校正。群体生理先验、逐图器官样
    摄取估计和病灶保护门控共同构成 Adaptive PHYSIO，不拆开作为独立模块。
    python train.py \
      --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/FDG \
      --load_ckpt \
      --experiment_name PhyTwin_PETCT_FDG \
      ...
      --memory_weight 0.0 \
      --gpu 6 \
      --save_dir ./ablation_fdg_m3_adaptive_physio

    M4. + 多尺度病灶感知推理
    作用：在 M3 的校正异常图基础上，将 image-level 推理从 top-1% 平均改为
    lesion_z，验证小病灶/多灶、PET-CT 不一致、生理先验重合、视野边缘伪影和
    大块器官样热点建模对图像级排序的贡献。
    python train.py \
      --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/PSMA \
      --load_ckpt \
      ...
      --memory_weight 0.0 \
      --gpu 5 \
      --save_dir ./ablation_psma_m4_full

    FDG 消融只需把 data_root、experiment_name 和 save_dir 换成 FDG：
      --data_root /data/cyf/shared_data/A-PETCT/2d_equal_mask50/FDG
      --experiment_name PhyTwin_PETCT_FDG
      --save_dir ./ablation_fdg_xxx
...
说明：
    ...
    python train.py \
      ...
      --failure_vis_num 20 \
      --gpu 5 \
      --save_dir ./saved_results_psma_failure
"""
...
def parse_args():
    ...
    parser.add_argument("--data_root", type=str, default="/data/cyf/shared_data/A-PETCT/2d_equal_mask50/PSMA")
...
def build_adaptive_physio(args):
    ...
    return AdaptivePhysioSuppressor(
        ...
        anomaly_quantile=args.adaptive_anomaly_quantile,
...
def should_use_memory(args):
    return float(args.memory_weight) > 0.0 or bool(args.use_hotspot_memory)


def memory_z_map(memory, residual, calibration, args):
    if not should_use_memory(args):
        return np.zeros(tuple(residual.shape[-2:]), dtype=np.float32)
    if memory is None:
        raise RuntimeError("memory_weight > 0 or hotspot memory requested, but residual memory is unavailable.")
    mem_map, _ = memory.score_map(residual)
    memory_np = mem_map[0].detach().cpu().numpy().astype(np.float32)
    return positive_zscore_map(memory_np, calibration["memory_mean"], calibration["memory_std"])
...
def image_refinement_config(args):
    return {
        ...
        "adaptive_anomaly_quantile": float(args.adaptive_anomaly_quantile),
...
def calibrate_normal_lesion_scores(model, memory, prior, loader, device, args, calibration, logger=None):
    model.eval()
    if should_use_memory(args):
        if memory is None:
            raise RuntimeError("Residual memory is required by this configuration but is not available.")
    ...
    if args.use_hotspot_memory:
        ...
        for _idx, batch in limited(loader, args.max_memory_batches):
            ...
            mem_map = memory.score_map(residual)[0] if should_use_memory(args) else None
            for b in range(residual.shape[0]):
                ...
                if mem_map is None:
                    memory_z = np.zeros_like(residual_z, dtype=np.float32)
                else:
...
def build_memory_and_prior(model, loader, device, args, logger):
    ...
    if should_use_memory(args):
        ...
        memory = None
        mem_maps = None
        calibration["memory_mean"] = 0.0
        calibration["memory_std"] = 1.0
    prior = None
    ...
    logger.info(
        "Built normal models | "
        f"normal_residuals={len(residuals)} | "
        f"memory_patches={0 if memory is None else memory.memory.shape[0]} | "
...
def load_full_checkpoint(path, model):
    ...
    memory = None if memory_state is None else ResidualPatchMemory().load_state_dict(memory_state)
    prior = None
...
def main():
    ...
    if args.load_ckpt:
        ...
        if args.no_physio:
            prior = None
            logger.info("Static PHYSIO prior disabled by --no_physio after checkpoint loading.")
