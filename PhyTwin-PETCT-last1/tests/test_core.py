...
class TestPhyTwinCore(unittest.TestCase):
    ...
    def test_adaptive_physio_suppresses_large_hotspot_more_than_small_focus(self):
        from phytwin_petct.physio import AdaptivePhysioSuppressor

        anomaly = np.ones((64, 64), dtype=np.float32)
        ...
        pet[48:51, 48:51] = 1.0
        anomaly[48:51, 48:51] = 3.0

        suppressor = AdaptivePhysioSuppressor(alpha=0.6, pet_quantile=0.95, min_area=6, sigma=0.0)
        out, mask = suppressor.suppress(anomaly, pet, ct_map=ct, static_prior=prior)

        self.assertGreater(float(mask[25, 25]), float(mask[49, 49]))
        self.assertLess(float(out[25, 25]), float(anomaly[25, 25]))
        self.assertGreater(float(out[49, 49] / anomaly[49, 49]), float(out[25, 25] / anomaly[25, 25]))
