...
"""
Run example:
cd /data/cyf/codes/A-PET-CT/INPFormer
...
from train import build_model, build_transforms, parse_modalities, resolve_device
...
TRACERS = ("PSMA", "FDG")
PANEL_NAMES = ("PET", "CT", "GT", "Pred", "INPFormer")
...
def checkpoint_for_tracer(args, tracer: str, modalities) -> Path:
    ...
    ckpt_dir = resolve_path(args.checkpoint_dir) / tracer.upper() / "_".join(modalities)
    for name in ("model.pth", "best_model.pth", "best_model.pt"):
        ckpt = ckpt_dir / name
        if ckpt.exists():
            return ckpt
    candidates = sorted(ckpt_dir.glob("*.pth")) + sorted(ckpt_dir.glob("*.pt"))
    if candidates:
        return candidates[-1]
    raise FileNotFoundError(f"No checkpoint found in {ckpt_dir}; provide --{tracer.lower()}_checkpoint")


def load_model_for_tracer(args, tracer: str, modalities, device):
    ckpt_path = checkpoint_for_tracer(args, tracer, modalities)
    args.tracer = tracer.upper()
...
def build_input(paths, modalities, data_transform, image_size):
    images = {mod: read_gray(paths[mod], image_size) for mod in modalities}
    ...
    return data_transform(image).unsqueeze(0), images


def render_case(args, model, ckpt_path, modalities, data_transform, eval_size, device, data_root, group, tracer, patient, slice_id):
    paths = case_paths(data_root, tracer, patient, slice_id)
    check_paths(paths)
    image, images = build_input(paths, modalities, data_transform, eval_size)
    image = image.to(device)
    with torch.no_grad():
        en, de, _ = model(image)
        raw_map = build_anomaly_map(model, en, de, eval_size, args.anomaly_source).squeeze().detach().cpu().numpy()
    ...
    save_image(case_dir / "INPFormer_heatmap.png", heatmap_img, args.image_size)
    ...
    return {"meta": meta, "panels": {"PET": pet_img, "CT": ct_img, "GT": gt_img, "Pred": pred_img, "INPFormer": heatmap_img}}
    ...
    for group in GROUPS:
        for tracer in TRACERS:
            ...
            records_by_group[group].append(
                render_case(args, model, ckpt_path, modalities, data_transform, eval_size, device, data_root, group, tracer, patient, slice_id)
            )
